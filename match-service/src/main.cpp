...
int main(int argc, char** argv) {
    std::string grpc_port = getEnvOrDefault("GRPC_PORT", "50052");
    ...
    std::string player_service_target = getEnvOrDefault("PLAYER_SERVICE_ADDR", "localhost:50051");
    ...
    std::cout << "[match_service] connecting to Postgres at " << pg_host << ":" << pg_port
              << ", player service at " << player_service_target << std::endl;
    ...
    auto player_client = std::make_shared<PlayerClient>(player_service_target);

    MatchServiceImpl service(db_pool, player_client);
    ...
    std::cout << "[match_service] listening on " << server_address << std::endl;
    server->Wait();
